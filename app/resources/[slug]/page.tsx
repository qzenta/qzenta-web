  ...
  return (
    <>
      <section className="px-4 sm:px-6 lg:px-8 pt-20 pb-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3 text-xs text-onyx-300 mb-5">
          <time dateTime={post.date}>{formatDate(post.date)}</time>
          <span>&middot;</span>
          <span>{post.readTime}</span>
        </div>

        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-onyx-50 leading-tight max-w-3xl">
          {post.title}
        </h1>
        <p className="mt-5 text-lg text-onyx-500 max-w-2xl leading-relaxed">{post.excerpt}</p>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-24 max-w-3xl mx-auto">
        <div className="border-t border-onyx-700 pt-10 space-y-5">
          {paragraphs.map((para, i) => {
            if (para.startsWith("## ")) {
              return (
                <h2 key={i} className="text-xl font-semibold text-onyx-50 pt-4">
                  {para.replace("## ", "")}
                </h2>
              );
            }
            if (para.startsWith("- ")) {
              ...
              return (
                <ul key={i} className="space-y-2">
                  {items.map((item, j) => (
                    <li key={j} className="flex items-start gap-3 text-onyx-500 text-sm leading-relaxed">
            ...
            return (
              <p key={i} className="text-onyx-500 leading-relaxed">
                {para}
              </p>
            );
          })}
