  ...
  return (
    <>
      <section className="px-4 sm:px-6 lg:px-8 pt-20 pb-8 max-w-7xl mx-auto">
        ...
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#f1f5f9] leading-tight max-w-3xl">
          {post.title}
        </h1>
        <p className="mt-5 text-lg text-[#94a3b8] max-w-2xl leading-relaxed">{post.excerpt}</p>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-24 max-w-3xl mx-auto">
        <div className="border-t border-[#2d4060] pt-10 space-y-5">
          {paragraphs.map((para, i) => {
            if (para.startsWith("## ")) {
              return (
                <h2 key={i} className="text-xl font-semibold text-[#f1f5f9] pt-4">
                  {para.replace("## ", "")}
                </h2>
              );
            }
